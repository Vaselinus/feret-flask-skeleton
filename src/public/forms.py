...
class CukrovinkyForm(Form):
    Druh = TextField('Druh cukrovinky', validators=[
        Predicate(safe_characters, message="Chybny znak!"),
        Length(min=3, max=20, message="Nazev musi mit od 3 do 20 pismen!"),
        InputRequired(message="Nesmi byt nevyplneno!")
    ])
    Cena =  TextField('Cena cukorvinky', validators=[
            Predicate(safe_characters, message="Zadej pouze cisla!"),
            Length(min=1, max=30, message="Cena musi byt dlouha od 1 do 30 cisel!"),
            InputRequired(message="Cena musi byt vyplnena!")
    ])
